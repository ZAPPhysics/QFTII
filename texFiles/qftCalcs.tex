...
\end{equation}
%Finally, we recall that we can use the cross section to get a rate by multiplying it by the reaction flux, $\mathcal{F}_\alpha = u_\alpha/V$. In the experiment
%frame, it is straightforward to verify that the relative velocity is
%\begin{equation}
%	u_\alpha = \frac{\sqrt{(p_1'\cdot p_2')^2 - m_\phi^2}}{E_0'{}^2} = \frac{|\mathbf{p}'|}{|\mathbf{p}'|^2 + m_\phi^2}
%		\sqrt{(1 + \cos\psi)(|\mathbf{p}'|^2(1+\cos\psi) + 2m_\phi^2)}\,,
%\end{equation}
%and using the reaction volume that we already found, we find the total reaction rate
%\begin{equation}\label{eq:ratelo}
%	R_{\phi^2 \to \theta^2} = \frac{\lambda_{\phi\theta}^2(1+\cos\psi)}{32\sigma_b^2(|\mathbf{p}'|^2 + m_\phi^2)\sqrt{|\mathbf{p}'|^2\cos^2\frac{\psi}{2} + m_\phi^2}}\,.
%\end{equation}
